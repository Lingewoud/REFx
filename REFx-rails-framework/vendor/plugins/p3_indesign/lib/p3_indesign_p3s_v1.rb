class P3Indesign_p3s_v1

	def initialize(p3s_string)
		@log	= ''
		@logger = P3Indesign_logger.new()
		@p3l	= initLang	
	...
	def initLang
		p3s_lang = P3Indesign_p3s_v1_lang.new
	...
	def checkBoolean(data)
		data = stripSpaces(data)
		if(data.strip == "false" || data.strip == "true")
			return data
		 else
			return false
		 end
	...
	def checkT3Path(data)
		#data = stripSpaces(data)
		data = data.strip
		if(data.to_s[0,6] == 'T3DOM:')
			return data
		else
			return false
		end

		#below old validation

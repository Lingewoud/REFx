class P3Indesign_library
	...
	def openDoc(filepath)
		log("Opening Indesign document:", filepath) 

		ret_doc	= @idApp.open(MacTypes::FileURL.path(filepath).hfs_path)

		log("Set preflight off","") 
...
#	end
	...
	def exec_exportPDF(doc,destpdf,preset)
		...
		log("exporting using preset",preset.to_s)
	...
	def exportPNGviaEPS(doc, orig, dest, pixWidth, pixHeight)
		if(!@dryrun)
			if(@idApp.to_s.downcase.match(/server/))
				@idApp.export(doc, :format => :EPS_type, :to => MacTypes::FileURL.path(orig).hfs_path, :timeout => 0)
			else
				@idApp.export(doc, :format => :EPS_type, :to => MacTypes::FileURL.path(orig).hfs_path, :showing_options => false, :timeout => 0)
			end
